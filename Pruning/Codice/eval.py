...
def iterative_pruning(args, net, train_loader, test_loader, x_prune, device,
    ...
    print('Numero iniziale della maschera: ', init_masks_num)
    init_flops = compute_flops(net)
    print('FLOPs iniziali: ', init_flops)
    ...
    for it in range(1, args.num_iters + 1):
        ...
        print('Numero di maschere dopo il pruning: ', after_masks_num)
        curr_flops = compute_flops(net)
        print('FLOPs dopo il pruning: ', curr_flops)
...
def eval(args):
    ...
    #salvo il file dei pesi dopo il pruning
    #for i in range(args.num_iters):
        #net._apply_mask()
        #weights_after_pruning = net.state_dict()
        #torch.save(weights_after_pruning, f'weights_after_pruning_iteration_{i}.pth')

    #codice per capire quali sono i nomi assegnati ai layer per i pesi
    #weights = torch.load('weights_after_pruning_iteration_0.pth')
